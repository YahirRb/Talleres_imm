...
export const MenuDefault = component$(() = > {
  ...
  return (<><div class = "cont_bienvenida"><p class = "bienvenida"> Buen dia{
      nombre}</ p></ div></>);
});
...
export const Mes = component$<MesesProps>((props) = > {
  if (props.elegido)
    return (<option value = {props.value} selected>{props.mes}</ option>);
...
const OptsMesesAgregar =
    Object.entries(meses).map(([ numeroMes, nombreMes ]) = > {
      const mesActual = new Date().getMonth();
      if (mesActual + 1 == Number(numeroMes) || mesActual + 1 == 13)
        return (<Mes key = {numeroMes} value = {Number(numeroMes)} mes = {
                     nombreMes} elegido = {true} />);
      return (<Mes key = {numeroMes} value = {Number(numeroMes)} mes = {
                   nombreMes} elegido = {false} />);
    });
...
export const ListaTalleres = component$(() => {
  ...
  const taller_ejemplo = {
    mes: "Diciembre", nombre: "Taller ejemplo", instructor: "Jane Doe", dias: "31", cupoMaximo: 5
  }
  ...
  return (
    ...
    opciones_taller.style.border =
        "1px solid var(--color_principal)" opciones_taller.style.borderLeft =
            "none"

            icono_expandido.classList
                .remove("bi-arrow-bar-right")
          ...
          <div class="opciones_taller" id="opciones_taller" >
            <span class="boton_taller" onClick$={() => {
              window.location.href = `/${JSON.stringify(taller_ejemplo)}/datos_taller`
            }} >
              <p>Editar taller</p>
